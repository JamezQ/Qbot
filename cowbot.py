...
cmd = """curl -b cookies.txt -c cookies.txt -H 'Content-Type: application/json' -d '{"CommandString": "login guy masturbate"}' http://u413.com/Terminal/ExecuteCommand 2> /dev/null"""
...
def Send(data): 
	...
	#cowsay_string = """{"CommandString": "channel QBOT """+data+ "\n\t(oo)\n\t(__)\       )\/\n\t    ||----w |\n\t    ||     ||" """"}\n"""
	cow_pic = """\\n ._______(o o) - """ + data + """\\n (_u413_\\/.(__) \\n .||....|| """
 	#cow_pic = """ likes dicks"""	
	cowsay_string = """{"CommandString": "channel QBOT """+cow_pic+""""}\n"""
	file.write(cowsay_string)
	file.close
...
while True:
	...
	if Chat is not None: #If new chat data exists
		for data in Chat:
			...
			if Command(m, "cowsay")[0]: #If the user send this command
